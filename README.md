...
## Какво е подобрено до версия 2.3
...
изпълнявай старите `migration_2.sql`–`migration_11.sql` след него.
...
supabase/migration_9.sql              основно защитно обновяване
supabase/migration_10.sql             свързване на акаунти с клиентски записи
supabase/migration_11.sql             дедупликация и надеждна смяна на роли
...
## Обновяване до версия 2.3

След `migration_10.sql` изпълнете `supabase/migration_11.sql`. Миграцията обединява
дублираните клиентски записи без загуба на измервания и гарантира точно един
`clients` запис за всеки акаунт с роля `trainer` или `client`.

След SQL миграцията публикувайте Edge Function-а и frontend-а отново:
...
В „Структура“ бутонът `⇄` сменя директно ролята между треньор и клиент.

## Версия 2.4

Версия 2.4 добавя администраторски пълен JSON backup/restore, видим бутон „Назад“, коригиран печат без празна първа област, дата до „Последна стойност“ и скриване на CSV импорт/експорт за клиентски акаунти.

За backup/restore трябва да бъде публикувана актуалната Edge Function:

```bash
supabase functions deploy manage-account --no-verify-jwt
```

Подробности: `CHANGES_2.4_BG.md`.


## Обновяване до версия 2.5

Версия 2.5 разпъва банера по цялата ширина, добавя видими бутони с флаговете 🇧🇬/🇬🇧 и въвежда отделни български и английски имена на параметрите.

След `migration_11.sql` изпълнете:

```text
supabase/migration_12.sql
```

Миграцията добавя `parameters.name_en`, превежда стандартните Tanita и телесни параметри и запазва потребителските параметри без загуба на данни. След това публикувайте актуалната Edge Function и frontend-а:
...
В „Настройки на параметрите“ администраторът редактира отделно българското и английското име. Стари CSV файлове продължават да се импортират, защото системата разпознава имената и на двата езика.
